...
unsigned int Dictionary::levenshteinDistance(const string word, const string s) const {
	string::size_type s_length {s.length()}, w_length {word.length()};
	int d[26][26];
	d[0][0] = 0;
	for (string_st i = 1; i <= w_length; ++i) d[i][0] = i;
	for (string_st j = 1; j <= s_length; ++j) d[0][j] = j;
	// Calculate cost using Levenshtein distance
	for (string_st i = 1; i <= w_length; ++i)
		for (string_st j = 1; j <= s_length; ++j)
			d[i][j] = min({ d[i-1][j-1] + (word[i-1] == s[j-1] ? 0 : 1), d[i-1][j] + 1,	d[i][j-1] + 1 });
	return d[w_length][s_length];
}
